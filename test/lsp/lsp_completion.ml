...
let%expect_test "datadiv-completion" =
  ...
  [%expect {|
    ...
    ----                        ^
       ...
       8             88 BB VALUES ARE "x" THRU "Z".
    (line 5, character 21):
    List of completions (73 entries): [VARYING;VALUES;VALUE;VAL-STATUS;USAGE;TYPEDEF;TYPE;TRAILING;SYNCHRONIZED;SIGN;SELECT;SAME;REDEFINES;PROPERTY;PROGRAM-POINTER;PROCEDURE-POINTER;PRESENT;POINTER;PICTURE;PACKED-DECIMAL;OCCURS;OBJECT;NATIONAL;LEADING;JUSTIFIED;IS TYPEDEF;IS GLOBAL;IS EXTERNAL;INVALID;INDEX;GROUP-USAGE;GLOBAL;FUNCTION-POINTER;FLOAT-SHORT;FLOAT-LONG;FLOAT-EXTENDED;FLOAT-DECIMAL-34;FLOAT-DECIMAL-16;FLOAT-BINARY-64;FLOAT-BINARY-32;FLOAT-BINARY-128;EXTERNAL;DYNAMIC;DISPLAY;DESTINATION;DEFAULT;COMP-X;COMP-N;COMP-9;COMP-6;COMP-5;COMP-4;COMP-3;COMP-2;COMP-15;COMP-10;COMP-1;COMP-0;COMP;CLASS;BLANK;BIT;BINARY-SHORT;BINARY-LONG;BINARY-DOUBLE;BINARY-C-LONG;BINARY-CHAR;BINARY;BASED;ANY;ALIGNED;RENAMES;CONSTANT]
    __rootdir__/prog.cob:7.8:
       4           DATA DIVISION.
       5           WORKING-STORAGE SECTION.
       6           01 DATA-NAME PIC X.
    ...
    ----                                  ^
      ...
      10             DISPLAY DATA-NAME.
    (line 7, character 31):
    List of completions (12 entries): [WHEN;TO;SET;IN;FALSE;ZEROS;SPACES;QUOTES;LOW-VALUES;HIGH-VALUES;ALL;THRU] |}]
...
let%expect_test "procedure-completion" =
  ...
  [%expect {|
    ...
    ----                        ^
      ...
      11             MULTIPLY 4 BY 2 GIVING BB
    (line 8, character 21):
    List of completions (127 entries): [WRITE;WITH NO ADVANCING;WITH DATA;WHEN;VALIDATE;UNSTRING;UNLOCK;TRANSFORM;TERMINATE;SUPPRESS;SUBTRACT;STRING;STOP;START;SORT;SET;SEND;SEARCH;REWRITE;RETURN;RESUME;RELEASE;RECEIVE;READ;RAISE;PURGE;PERFORM;OVERFLOW;OPEN;ON SIZE ERROR;ON OVERFLOW;ON EXCEPTION;NOT ON SIZE ERROR;NOT ON OVERFLOW;NOT ON EXCEPTION;NOT INVALID KEY;NOT AT EOP;NOT AT END;NEXT SENTENCE;MULTIPLY;MOVE;MERGE;INVOKE;INVALID KEY;INSPECT;INITIATE;INITIALIZE;IF;GOBACK;GO;GENERATE;FREE;EXIT;EXCEPTION;EVALUATE;EOP;ENTRY;ENTER;END-WRITE;END-UNSTRING;END-SUBTRACT;END-STRING;END-START;END-SEARCH;END-REWRITE;END-RETURN;END-RECEIVE;END-READ;END-PERFORM;END-OF-PAGE;END-MULTIPLY;END-IF;END-EVALUATE;END-DIVIDE;END-DISPLAY;END-DELETE;END-COMPUTE;END-CALL;END-ADD;END-ACCEPT;END;ENABLE;ELSE;DIVIDE;DISPLAY;DISABLE;DELETE;DATA;CONTINUE;COMPUTE;CLOSE;CANCEL;CALL;AT EOP;AT END;ALTER;ALLOCATE;ADD;ACCEPT;WITH;UPON;POSITION;MODE;LINE;COLUMN;COL;AT;AA;BB;ZEROS;SUPER;SPACES;SELF;QUOTES;PAGE-COUNTER;NULL;LOW-VALUES;LINE-COUNTER;LINAGE-COUNTER;HIGH-VALUES;FUNCTION;EXCEPTION-OBJECT;ALL;ADDRESS;AS;OF;IN]
    ...
    ----                     ^
      ...
      12               ON SIZE ERROR
    (line 9, character 18):
    List of completions (4 entries): [TO;AS;OF;IN]
    ...
    List of completions (4 entries): [AS;BY;OF;IN]
    ...
    List of completions (113 entries): [WRITE;WITH DATA;WHEN;VALIDATE;UNSTRING;UNLOCK;TRANSFORM;TERMINATE;SUPPRESS;SUPER;SUBTRACT;STRING;STOP;START;SORT;SET;SEND;SELF;SEARCH;ROUNDED;REWRITE;RETURN;RESUME;REMAINDER;RELEASE;RECEIVE;READ;RAISE;PURGE;PERFORM;PAGE-COUNTER;OVERFLOW;OPEN;ON SIZE ERROR;ON OVERFLOW;ON EXCEPTION;NULL;NOT ON SIZE ERROR;NOT ON OVERFLOW;NOT ON EXCEPTION;NOT INVALID KEY;NOT AT EOP;NOT AT END;NEXT SENTENCE;MULTIPLY;MOVE;MERGE;LINE-COUNTER;LINAGE-COUNTER;INVOKE;INVALID KEY;INSPECT;INITIATE;INITIALIZE;IF;GOBACK;GO;GENERATE;FUNCTION;FREE;EXIT;EXCEPTION-OBJECT;EXCEPTION;EVALUATE;EOP;ENTRY;ENTER;END-WRITE;END-UNSTRING;END-SUBTRACT;END-STRING;END-START;END-SEARCH;END-REWRITE;END-RETURN;END-RECEIVE;END-READ;END-PERFORM;END-OF-PAGE;END-MULTIPLY;END-IF;END-EVALUATE;END-DIVIDE;END-DISPLAY;END-DELETE;END-COMPUTE;END-CALL;END-ADD;END-ACCEPT;END;ENABLE;ELSE;DIVIDE;DISPLAY;DISABLE;DELETE;DATA;CONTINUE;COMPUTE;CLOSE;CANCEL;CALL;AT EOP;AT END;AS;ALTER;ALLOCATE;ADDRESS;ADD;ACCEPT;GIVING;OF;IN]
    ...
    List of completions (110 entries): [WRITE;WITH DATA;WHEN;VALIDATE;UNSTRING;UNLOCK;TRANSFORM;TERMINATE;SUPPRESS;SUBTRACT;STRING;STOP;START;SORT;SET;SEND;SEARCH;REWRITE;RETURN;RESUME;RELEASE;RECEIVE;READ;RAISE;PURGE;PERFORM;OVERFLOW;OPEN;ON SIZE ERROR;ON OVERFLOW;ON EXCEPTION;NOT ON SIZE ERROR;NOT ON OVERFLOW;NOT ON EXCEPTION;NOT INVALID KEY;NOT AT EOP;NOT AT END;NEXT SENTENCE;MULTIPLY;MOVE;MERGE;INVOKE;INVALID KEY;INSPECT;INITIATE;INITIALIZE;IF;GOBACK;GO;GENERATE;FREE;EXIT;EXCEPTION;EVALUATE;EOP;ENTRY;ENTER;END-WRITE;END-UNSTRING;END-SUBTRACT;END-STRING;END-START;END-SEARCH;END-REWRITE;END-RETURN;END-RECEIVE;END-READ;END-PERFORM;END-OF-PAGE;END-MULTIPLY;END-IF;END-EVALUATE;END-DIVIDE;END-DISPLAY;END-DELETE;END-COMPUTE;END-CALL;END-ADD;END-ACCEPT;END;ENABLE;ELSE;DIVIDE;DISPLAY;DISABLE;DELETE;DATA;CONTINUE;COMPUTE;CLOSE;CANCEL;CALL;AT EOP;AT END;ALTER;ALLOCATE;ADD;ACCEPT;AA;BB;SUPER;SELF;PAGE-COUNTER;NULL;LINE-COUNTER;LINAGE-COUNTER;FUNCTION;EXCEPTION-OBJECT;ADDRESS;MODE]
    ...
    List of completions (109 entries): [WRITE;WITH DATA;WHEN;VALIDATE;UNSTRING;UNLOCK;TRANSFORM;TERMINATE;SUPPRESS;SUBTRACT;STRING;STOP;START;SORT;SET;SEND;SEARCH;REWRITE;RETURN;RESUME;RELEASE;RECEIVE;READ;RAISE;PURGE;PERFORM;OVERFLOW;OPEN;ON SIZE ERROR;ON OVERFLOW;ON EXCEPTION;NOT ON SIZE ERROR;NOT ON OVERFLOW;NOT ON EXCEPTION;NOT INVALID KEY;NOT AT EOP;NOT AT END;NEXT SENTENCE;MULTIPLY;MOVE;MERGE;INVOKE;INVALID KEY;INSPECT;INITIATE;INITIALIZE;IF;GOBACK;GO;GENERATE;FREE;EXIT;EXCEPTION;EVALUATE;EOP;ENTRY;ENTER;END-WRITE;END-UNSTRING;END-SUBTRACT;END-STRING;END-START;END-SEARCH;END-REWRITE;END-RETURN;END-RECEIVE;END-READ;END-PERFORM;END-OF-PAGE;END-MULTIPLY;END-IF;END-EVALUATE;END-DIVIDE;END-DISPLAY;END-DELETE;END-COMPUTE;END-CALL;END-ADD;END-ACCEPT;END;ENABLE;ELSE;DIVIDE;DISPLAY;DISABLE;DELETE;DATA;CONTINUE;COMPUTE;CLOSE;CANCEL;CALL;AT EOP;AT END;ALTER;ALLOCATE;ADD;ACCEPT;AA;BB;SUPER;SELF;PAGE-COUNTER;NULL;LINE-COUNTER;LINAGE-COUNTER;FUNCTION;EXCEPTION-OBJECT;ADDRESS]
    ...
    List of completions (124 entries): [WRITE;WITH NO ADVANCING;WITH DATA;WHEN;VALIDATE;UNSTRING;UNLOCK;TRANSFORM;TERMINATE;SUPPRESS;SUBTRACT;STRING;STOP;START;SORT;SET;SEND;SEARCH;REWRITE;RETURN;RESUME;RELEASE;RECEIVE;READ;RAISE;PURGE;PERFORM;OVERFLOW;OPEN;ON SIZE ERROR;ON OVERFLOW;ON EXCEPTION;NOT ON SIZE ERROR;NOT ON OVERFLOW;NOT ON EXCEPTION;NOT INVALID KEY;NOT AT EOP;NOT AT END;NEXT SENTENCE;MULTIPLY;MOVE;MERGE;INVOKE;INVALID KEY;INSPECT;INITIATE;INITIALIZE;IF;GOBACK;GO;GENERATE;FREE;EXIT;EXCEPTION;EVALUATE;EOP;ENTRY;ENTER;END-WRITE;END-UNSTRING;END-SUBTRACT;END-STRING;END-START;END-SEARCH;END-REWRITE;END-RETURN;END-RECEIVE;END-READ;END-PERFORM;END-OF-PAGE;END-MULTIPLY;END-IF;END-EVALUATE;END-DIVIDE;END-DISPLAY;END-DELETE;END-COMPUTE;END-CALL;END-ADD;END-ACCEPT;END;ENABLE;ELSE;DIVIDE;DISPLAY;DISABLE;DELETE;DATA;CONTINUE;COMPUTE;CLOSE;CANCEL;CALL;AT EOP;AT END;ALTER;ALLOCATE;ADD;ACCEPT;WITH;UPON;POSITION;MODE;LINE;COLUMN;COL;AT;AA;BB;ZEROS;SUPER;SPACES;SELF;QUOTES;PAGE-COUNTER;NULL;LOW-VALUES;LINE-COUNTER;LINAGE-COUNTER;HIGH-VALUES;FUNCTION;EXCEPTION-OBJECT;ALL;ADDRESS] |}]
